...
void grid::LoadShapes(ifstream& InFile)
{
	...
	for (int i = 0; i < shapecount; i++)
	{
		...
		{
		case SIGN:
			sh = new Sign(pGame, pt, clr);
			break;
		case ICECREAM:
			sh = new IceCream(pGame, pt, clr);
			break;
		case HOME:
			sh = new Home(pGame, pt, clr);
			break;
		case ROCKET:
			sh = new Rocket(pGame, pt, clr);
			break;
		case  FISH:
			sh = new Fish(pGame, pt, clr);
			break;
		case CAR:
			sh = new Car(pGame, pt, clr);
			break;
		case WATCH:
			sh = new Watch(pGame, pt, clr);
		}
		sh->Load(InFile);
		addShape(sh);
	}
